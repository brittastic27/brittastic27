# Hi there, I’m Brittney 👋

🎓 Computer Science sophomore at Princeton University  
🎨 Interested in HCI, UI/UX design, product design, frontend & full-stack development, and video games  
🧩 I enjoy building interactive, user-centered software and learning by doing
🎯 Fun fact: I spent summer '25 in Tokyo, Japan working at an IT consulting and web services company (I also explored, ate a lot of 7/11, and learned a bit of basic Japanese)
---

## 🛠️ Skills & Tools
- **Languages:** JavaScript, Java, Python, R, C, Swift
- **Frontend:** HTML, CSS, React (learning), SwiftUI  
- **Backend:** Flask, basic APIs, authentication, local databases (SQL)  
- **Tools:** Git/GitHub, XCode  
- **Courses:** Algorithms & Data Structures, Intro to Programming Systems, Reasoning about Computation

---

## 🌱 Currently Exploring
- Human-Computer interaction & usability design
- Frontend frameworks and modern web and mobile app development
- Bridging technical implementation with thoughtful product design
  
---

## 📫 Connect w/ Me
- LinkedIn: [linkedin.com/in/yourname](https://linkedin.com/in/yourname)
- School Email: bb2086@princeton.edu
